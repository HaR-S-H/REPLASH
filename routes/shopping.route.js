...
router.post("/", upload.array("images"),async (req, res) => {
  try { 
    ...
    const prompt = `
      Based on the provided images of vegetables, generate a list of two dishes that can be prepared using these ingredients. but mind like if i given a image of dosa you should give both results of dosa but different combination like rawa dosa simple dosa like this and Provide the response in the following exact JSON structure as an array of objects, where each object represents a dish:

      [
        {
          "name": "<Dish Name  give dish name very short not long >",
          "imageUrl": "<leave it empty string>",
          "nutritionalInfo": {
          ...
          "recipe": "<Step-by-step preparation instructions every point in next line >",
          "ingredientsUsed": ["<Ingredient 1>", "<Ingredient 2>", "<Ingredient 3>"],
          "cuisineType": "<Cuisine Type (e.g., Indian, Italian, Mexican)>"
        }
      ]

      Ensure the response contains only the JSON array and no additional text or explanations and recipe should come in points (.) instead of numbers and after completion of every ending of one step add - and add dot (.) in starting of every points and add imageUrl field also as i mentioned leave it with emtpy string.
    `;
    console.log(req.files);
